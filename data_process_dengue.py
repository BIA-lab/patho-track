...
def convert_omicron_to_lineages(df):
    aux_name = []
    for index, row in df.iterrows():
        if row['variant'].startswith("Omicron"):
            if row['lineage'].startswith("BA.1"):
                aux_name.append("Omicron (BA.1)")
            elif row['lineage'].startswith("BA.2"):
                aux_name.append("Omicron (BA.2)")
            # elif row['lineage'].startswith("BA.3"):
            #     aux_name.append("Omicron (BA.3)")
            elif row['lineage'].startswith("BA.4"):
                aux_name.append("Omicron (BA.4)")
            elif row['lineage'].startswith("BA.5"):
                aux_name.append("Omicron (BA.5)")
            elif row['lineage'].startswith("BQ"):
                aux_name.append("Omicron (BQ.*)")
            elif row['lineage'].startswith("XBB.1.5"):
                aux_name.append("Omicron (XBB.1.5)")
            elif row['lineage'].startswith("XBB") and ~row['lineage'].startswith("XBB.1.5"):
                aux_name.append("Omicron (XBB.*)")
            else:
                aux_name.append("Other Omicron")
        else:
            aux_name.append(row['variant'])
    df['variant'] = aux_name
    return df
...
def process_data_from_gisaid_api(base_url, username, password):
    ...
    # Convert omicron into different lineages
    df = convert_omicron_to_lineages(df)

    # Drop itens where collection date is before variant_cutoff (First sampled date)
